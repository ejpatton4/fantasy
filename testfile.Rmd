...
names(final_standings) <- c("Team","Wins","Losses","Win Pct","Points For","Points Against","Avg Points","Std. Dev. Points","Expected Wins","Expected Losses","Expected Win Pct")

final_standings <- final_standings %>%
  arrange(-`Win Pct`,-`Points For`)
...
final_standings <- final_standings %>%
  mutate(Luck = `Win Pct`- `Expected Win Pct`)

final_standings$`Avg Points` <- round(final_standings$`Avg Points`,digits = 2)

final_standings$`Std. Dev. Points` <- round(final_standings$`Std. Dev. Points`,digits = 2)
...
*  Std. Dev. Points - You can read about standard deviation [here](https://en.wikipedia.org/wiki/Standard_deviation), but essentially it is a measure of how consistent the team scores. Higher means more volatile, lower means often score close to the teams average.  
...
datatable(final_standings %>%
            filter(`Std. Dev. Points` == max(`Std. Dev. Points`)) %>%
            select(Team,`Std. Dev. Points`))
...
datatable(final_standings %>%
            filter(`Std. Dev. Points` == min(`Std. Dev. Points`)) %>%
            select(Team,`Std. Dev. Points`))
